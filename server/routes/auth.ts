...
router.post('/login', async (req, res) => {
  try {
    ...
    return res.json({ token, usuario: { ...userData, id: String(userData.id), dataCadastro: userData.dataCadastro.toISOString() } });
  } catch (err) {
    console.error(err);
...
router.post('/register', async (req, res) => {
  try {
    ...
    const { senhaHash: _, ...userData } = newUser;
    return res.status(201).json({ token, usuario: { ...userData, id: String(userData.id), dataCadastro: userData.dataCadastro.toISOString() } });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: 'Erro interno do servidor' });
  }
